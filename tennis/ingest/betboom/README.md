...
| `client.py` | The recorder: subscribes to live tennis, writes every frame raw, follows the pushed snapshots. |
...
full snapshot roughly every 13 s (measured live; the first draft here said one
message per price change, which was read off the schema and turned out wrong).
