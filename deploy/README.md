...
| `APP_BUILD` конторы сменился | схема protobuf могла устареть, поля — молча разъехаться | `python -m tennis.ingest.betboom.check_build`: код 0 — схема та же (25.09 так было с 8.45.2), код 1 — список сдвинутых полей; тогда пересобрать схему, рецепт в `docs/TRACK_A_betboom_capture.md` |
| `last_disconnect` — `3003 ... Access rejected` с любого адреса, подписок 0 | фид сменил uuid сокета: 24.09 он начал требовать `?uuid=` виджета, и запись стояла 29 ч | `python -m tennis.ingest.betboom.check_build`; если «uuid сокета сменился» — новый uuid в `DEFAULT_URL` (`tennis/ingest/betboom/client.py`), коммит, `git pull` и рестарт на сервере |
