...
| `reconnects` +3…10 за час, в `last_disconnect` — `no close frame received or sent` или `did not receive a valid HTTP response`, котировки при этом идут | фид рвёт сессии на своей стороне: 23.09 с 15:20 МСК, у второго адреса в те же минуты | на машине ничего не делать: рекордер переподключается сам, котировки снова идут через ~3 с. Во что обошлись обрывы — поле `blind_s` в отчёте (секунды без котировок за запуск). Если за час оно выросло больше чем на минуту — присылать `journalctl -u betboom-capture -n 50 --no-pager` |
